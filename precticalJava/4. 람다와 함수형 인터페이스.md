...
###4.6 메서드 참조
함수를 메서드의 파라미터로 전달하는 것을 메서드 참조(methoda reference) 라고 한다.
*장점 : 람다표현식과 달리 여러곳에서 재사용 할 수 있고 자바의 기본 제공 메서드뿐 아니라 직접 개발한 메서드도 사용할 수 있다.
메서드 참조는 함수형 인터페이스와 연관되어 있지만 람다표현식을 대체하기 위한 것이 아닌 상호보완적인 관계다.
