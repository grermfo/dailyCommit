...
인터페이스에 default, private, static메서드가 포함되면서 상속과 유사한 형태가 되었다. 실제로 특정 인터페이스에 정의 되어 있는
...
Male과 Female을 통해 하위 인터페이스는 상위 인터페이스의 메서드를 오버라이드해서 default메서드로 재정의 할수 있다.
그럼 Male과 Female인터페이스를 구현한 Work 클래스를 작성해 보자
~~~java
...
자바8에서는 중복된  default메서드를 상속받지 못하도록 컴파일러에서 차단하고 있다.
...
super키워드를 이용해서 직접 인터페이스 default메서드를 호출하는 방식은 2가지 이다.
* 클래스 작성시 구현할 인터페이스의 default메서드를 오버라이드하지 않을 경우 직접 default메서드를 호출할 수 있다.
* 클래스의 생성자 혹은 메서드에서 구현할 인터페이스의 default메서드는 super 키워드를 통해 호출 할 수 있다.
...
Worker클래스 수정. 이렇게 소스코드를 변경하면  Worker클래스는 오류가 나지 않음.
...
인터페이스에서 default메서드를 제공하면서 제한적으로 자바에 다중 상속이 가능해 졌다.
